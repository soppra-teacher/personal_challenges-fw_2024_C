...
/**
 * 収支一覧（個別）画面削除アクションクラス
 * @author soppra
 */
public class ShushiListKobetsuDeleteAction extends BaseAction {
	/** Service **/
	...
	protected ActionForward doProcess(ActionMapping map, ActionForm form,
		...
		// 削除
		shushiService.shushiListKobetsuDelete(formMap);

		// メッセージ表示
		request.getSession().setAttribute(SESSION_LIST_MESSAGE_SHUSHI_KOBETSU, MSG_SUCCESS_DELETE);
	...
	private boolean check(Map<String, Object> formMap, HttpServletRequest request) {
		...
		// 選択レコードが無ければエラー
		String[] checkDel = (String[]) formMap.get(MSG_ERRORS_NO_DELETE);
