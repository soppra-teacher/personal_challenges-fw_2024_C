package cashbook.util;

/**
 * 共通定数クラス(個人用)
 * @author soppra
 */
public class SensekiConst {
	//public static final String  FORM_KOJIN_LIST = "sensekiListForm";
	/** フォーム名(マスタメンテ) */
	public static final String FORM_SENSEKI_LIST = "sensekiListForm";
	/** フォーム名(マスタ登録) */
	public static final String FORM_SENSEKI_REGIST = "sensekiRegistForm";
	
	public static final String SENSEKI_LIST_DTO="SENSEKI_LIST_DTO";

	/** キー値：個人ID */
	public static final String KEY_SENSEKI_ID = "sensekiId";
	/** キー値：個人名 */
	public static final String KEY_SENSEKI_NM = "sensekiNm";
	/** キー値：個人名カナ */
	public static final String KEY_SENSEKI_NM_KANA = "sensekiNmkana";
	/** キー値：世帯主フラグ */
	public static final String KEY_SETAINUSI_FLG = "setaiNusiFlg";
	/** キー値：世帯主フラグ値 */
	public static final String KEY_SETAINUSI_FLG_VALUE = "setaiNusiFlgValue";
	/** キー値：続柄キー */
	public static final String KEY_ZOKUGARA = "zokugaraKey";
	/** キー値：性別区分キー */
	public static final String KEY_SEIBETSU_KBN_KEY = "seibetsuKbnKey";
	/** キー値：性別区分 */
	public static final String KEY_SEIBETSU_KBN = "seibetsuKbn";
	/** キー値：パス */
	public static final String KEY_PASS = "pass";
	
	public static final String KEY_SITTEN = "sittenNm";
	public static final String KEY_JISEKITEN = "jisekitenNm";
	public static final String KEY_E_TEAM = "eTeam";
	public static final String KEY_MATCH_DATE = "matchDate";
	
	//senshuId
	public static final String KEY_SENSHUID = "senshuId";
	
	
}
